  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        fontFamily: 'Roboto',
        cardTheme: CardTheme(elevation: 0, color: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24))),
      ),
      home: const AppShell(),
    );
  }
...
class Store extends ChangeNotifier {
  ...
  Future<String> importJson() async { final res=await FilePicker.platform.pickFiles(type:FileType.custom, allowedExtensions:['json'], withData:true); if(res==null) return 'لم يتم اختيار ملف'; final txt=utf8.decode(res.files.single.bytes!); final data=jsonDecode(txt); final list=(data is List?data:data['words']) as List; words=list.map((e)=>WordItem.fromJson(e)).toList(); await _persistWords(); notifyListeners(); return 'تم استيراد ${words.length} كلمة من JSON'; }
  Future<String> importExcel() async { final res=await FilePicker.platform.pickFiles(type:FileType.custom, allowedExtensions:['xlsx'], withData:true); if(res==null) return 'لم يتم اختيار ملف'; final excel=ex.Excel.decodeBytes(res.files.single.bytes!); final newWords=<WordItem>[]; int idStart=(words.map((e)=>e.id).fold(0, max))+1; for(final table in excel.tables.values){ for(int i=1;i<table.rows.length;i++){ final row=table.rows[i]; if(row.length<3) continue; final grade=row[0]?.value?.toString() ?? ''; final en=row[1]?.value?.toString() ?? ''; final ar=row[2]?.value?.toString() ?? ''; if(grade.trim().isNotEmpty && en.trim().isNotEmpty && ar.trim().isNotEmpty){ newWords.add(WordItem(id:idStart++, grade:grade.trim(), en:en.trim(), ar:ar.trim(), source:'excel')); } } }
    if(newWords.isEmpty) return 'لم أجد كلمات. ترتيب الأعمدة يجب أن يكون: grade, word_en, meaning_ar';
    final seen=words.map((e)=>e.en.toLowerCase()).toSet(); for(final w in newWords){ if(!seen.contains(w.en.toLowerCase())){ words.add(w); seen.add(w.en.toLowerCase()); }} await _persistWords(); notifyListeners(); return 'تم استيراد Excel. العدد الحالي: ${words.length} كلمة'; }
}
...
class SettingsPage extends StatelessWidget{ final Store store; const SettingsPage(this.store,{super.key}); @override Widget build(BuildContext c)=>PageFrame(title:'الإعدادات والتواصل', child:ListView(padding:const EdgeInsets.all(16),children:[SwitchListTile(title:const Text('أصوات الضغط والتفاعل'),value:store.sound,onChanged:(v)async{store.sound=v; await store.prefs!.setBool('sound', v); store.notifyListeners();}),Card(child:ListTile(leading:const Icon(Icons.update),title:const Text('تحديث القاموس من GitHub JSON'),onTap:()async{try{final m=await store.updateFromUrl(); if(c.mounted)msg(c,m);}catch(e){if(c.mounted)msg(c,'فشل التحديث. غيّر رابط JSON داخل main.dart لاحقاً.');}})),Card(child:ListTile(leading:const Icon(Icons.file_open),title:const Text('استيراد JSON'),onTap:()async{msg(c,await store.importJson());})),Card(child:ListTile(leading:const Icon(Icons.table_chart),title:const Text('استيراد Excel'),subtitle:const Text('الأعمدة: grade, word_en, meaning_ar'),onTap:()async{msg(c,await store.importExcel());})),Card(child:ListTile(leading:const Icon(Icons.picture_as_pdf),title:const Text('استيراد PDF'),subtitle:const Text('حوّل PDF إلى JSON/Excel ثم استورده هنا'),onTap:()=>msg(c,'PDF يحتاج استخراج كلمات أولاً. أرسل PDF لي وأنا أحوله إلى JSON مناسب.'))),Card(child:ListTile(leading:const Icon(Icons.email),title:const Text('إرسال بريد للمطور'),onTap:()=>launchUrl(Uri.parse('mailto:$developerEmail?subject=قاموسي المدرسي')))),Card(child:ListTile(leading:const Icon(Icons.report),title:const Text('اقتراح كلمة / الإبلاغ عن خطأ'),onTap:()=>launchUrl(Uri.parse('mailto:$developerEmail?subject=اقتراح أو تصحيح في قاموسي المدرسي')))),const Padding(padding:EdgeInsets.all(12),child:Text('الإصدار 1.0.0 - بدون إشعارات - أماكن الإعلانات جاهزة وغير مفعلة'))]));}
